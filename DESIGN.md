...
## TCC Clinic Compatibility Profile (Controlling)

This file was initially generated by `getdesign@0.6.25` with the `claude` preset. The generated Claude analysis remains below as design inspiration and provenance, but it is not a direction to copy Anthropic branding into this application. For this repository, this compatibility profile takes precedence wherever the generated reference conflicts with the existing TCC Clinic Management System.

### Product character and visual authority

- Design for a secure, practical clinic operations application, not a marketing site or AI chat product.
- Preserve TCC identity: the existing logo, maroon accent family, neutral white/light-gray work surfaces, and the existing dark theme are the current product language.
- Do not use Anthropic names, logos, spike marks, coral branding, or Claude-specific product patterns.
- Do not add Copernicus, Tiempos Headline, StyreneB, or another licensed font. The implemented system stack in `frontend/src/index.css` (`system-ui`, `Segoe UI`, Roboto, sans-serif) is authoritative until the project explicitly approves a font change.
- Existing implementation is evidence, not permission to multiply inconsistencies. Reuse the closest established component or treatment; propose any token consolidation as a focused change rather than silently restyling the application.

### Implemented TCC foundation

| Concern | Current project rule |
|---|---|
| Primary identity | TCC maroon. Existing screens use `#70091f`, `#8b1505`, and the older `#800020` family. Prefer the token or treatment already used by the surrounding module; do not introduce another near-duplicate maroon. |
| Light surfaces | Page floor near `#f7f8fb`; white cards; warm pale-maroon borders and highlights such as `#f0ded9`, `#fcebe7`, and `#fdf8f7`. |
| Dark surfaces | The `.tcc-dark` rules in `frontend/src/index.css` control the implemented dark theme, using near-black/navy surfaces, light text, and subdued slate secondary text. |
| Typography | System sans for interface text. Use clear hierarchy, readable weights, and compact operational labels; do not introduce editorial serif display typography. |
| Shape | Controls generally use 8-12px radii (`rounded-lg`/`rounded-xl`); primary cards and module headers commonly use 16px (`rounded-2xl`); pills and avatars may be fully rounded. |
| Depth | Use borders and restrained `shadow-sm` first. Reserve stronger shadows for overlays, active floating controls, or deliberate emphasis. |
| Icons | Reuse `lucide-react`; keep stroke style and sizing consistent with adjacent controls. Do not mix icon libraries without approval. |
| Desktop shell | Preserve the existing 264px sidebar and content offset defined by `MainLayout.jsx`. |
| Mobile shell | Preserve the fixed 64px header, drawer/overlay navigation, and 320px minimum supported viewport. |
| Responsive data | Tables must live in an overflow container on narrow screens. Forms and action groups must stack or wrap without horizontal page overflow. |

### Components and interaction

- Reuse `MainLayout`, `Sidebar`, module headers, cards, buttons, form controls, status treatments, empty states, and table patterns before creating variants.
- Keep operational pages scan-friendly: strong page title, short context, obvious primary action, clear filters, readable tables/cards, and explicit loading, empty, error, success, and disabled states.
- Primary actions use TCC maroon with legible white text. Destructive actions must be visually and semantically distinct; never use the normal primary treatment to hide destructive impact.
- Inputs require visible labels, validation feedback, and a keyboard-visible focus state. Do not rely on placeholder text as the only label.
- Interactive controls must use semantic HTML, descriptive accessible names, sensible focus order, and a minimum practical touch target of about 40px; prefer 44px where layout permits.
- Preserve Escape-to-close and overlay dismissal for mobile navigation and dialogs. Focus must be managed when adding modal interactions.
- Never communicate status by color alone. Pair color with text and, where useful, a Lucide icon.
- Respect `prefers-reduced-motion`. Motion should explain state change, remain brief, and never block clinic work.
- Dark-mode changes must be checked in both themes. Do not solve new dark-mode cases by adding broad selectors that unintentionally recolor unrelated modules.

### Layout and responsive behavior

- Follow the existing Tailwind CSS 4 utility approach and the global layers in `frontend/src/index.css`.
- Use mobile-first layouts and the breakpoints already present in nearby code, especially `sm` and `lg`. Do not invent a parallel breakpoint system without a demonstrated need.
- Keep page content full-width within the application shell. Apply a max width only when it improves a focused form or reading task.
- Dense clinical data may use horizontal table scrolling; do not shrink text below readable sizes merely to fit every column.
- Avoid fixed heights for content regions unless the interaction requires them. Account for phone browser toolbars with the existing `100dvh` pattern.

### Safety and content

- Clinic and identity data are sensitive. Avoid decorative exposure of patient information, and do not put real personal or medical data into fixtures, screenshots, examples, or documentation.
- Use plain, professional, non-alarming language. Confirmations must name the action and its consequence; errors should explain recovery without exposing server internals.
- Do not invent clinic policy, medical guidance, status meanings, retention rules, or institutional branding details. Mark an unapproved addition `PROPOSED` and request a decision when it affects behavior or meaning.

### Design workflow

1. Read this compatibility profile, then inspect the target page and its neighboring components.
2. Reuse the implemented product pattern when one exists.
3. If the generated Claude reference below conflicts with TCC implementation, follow this profile.
4. For a new pattern, document it as `PROPOSED`, implement the smallest reusable version, and include responsive, keyboard, focus, contrast, loading, error, empty, and dark-mode checks as applicable.
5. A broad visual redesign, palette normalization, typography replacement, or shell change requires explicit approval and should not be bundled into an unrelated feature.
